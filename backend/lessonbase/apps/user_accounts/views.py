...
class StudentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.polymorphic_ctype.name == "teacher":
            return Student.objects.filter(teacher=user).prefetch_related("class_groups")
        else:
            return Student.objects.filter(id=user.id).prefetch_related("class_groups")
    ...
    if serializer.is_valid():
        ...
        if user_type == 1:
            user = Teacher.objects.create(
                username=serializer.validated_data["username"], hire_date=datetime.now()
            )
            subject_ids = serializer.validated_data.get(
                "subjects", []
            )  # Get list of subject IDs from request data
...
def login(request):
    ...
    if serializer.is_valid(raise_exception=True):
        ...
        if user.polymorphic_ctype.model == "teacher":
            return Response(
                {"token": f"{token}", "user_type": "teacher"}, status=status.HTTP_200_OK
            )
        else:
            return Response(
                {"token": f"{token}", "user_type": "student"}, status=status.HTTP_200_OK
            )
    else:
        return Response(
...
def students_for_teacher(request):
    queryset = Student.objects.filter(teacher=request.user.id).prefetch_related(
        "class_groups"
    )
    serializer = StudentSerializer(queryset, many=True)
    return Response(serializer.data)
...
def new_student(request):
    ...
    msg_plain = render_to_string(
        ...
        {"token": token, "confirm_account": f"{BASE_URL}/confirm_account/"},
    )
...
def confirm_account(request):
    if request.method == "GET":
        ...
        if request.data["username"] and request.data["password1"]:
            ...
            return Response(
                {"error": "No username or password"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ClassGroupViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not teacher_list:
            ...
            (
                data["teachers"].append(self.request.user.pk)
                if self.request.user.pk not in data["teachers"]
                else None
            )
        serializer = self.get_serializer(data=data)
        if not serializer.is_valid(raise_exception=True):
