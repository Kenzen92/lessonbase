...
def check_assignment_graded_status(sender, instance, created, **kwargs):
    ...
    """
    print(f"[DEBUG] Signal triggered for AssignmentAttempt ID {instance.id}, created={created}")
    ...
    for eachStudent in all_assignment_students:
        ...
        if attempts:
            ...
            break
    if all_graded:
        print(f"[DEBUG] All students have graded attempts for Assignment ID {assignment.id}")
        assignment.marked = True
    else:
        assignment.marked = False
    assignment.save()
    print(f"[DEBUG] Assignment ID {assignment.id} saved with marked={assignment.marked}, to_mark={assignment.to_mark}")