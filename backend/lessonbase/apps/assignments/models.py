...
class Assignment(Model):
    ...
    subject = ForeignKey(
        Subject,
        on_delete=CASCADE,
        help_text="The subject to which this assignment belongs.",
    )
    teachers = ManyToManyField(
        CustomUser,
        related_name="assignments_as_teacher",
        help_text="Teachers assigning this work.",
    )
    students = ManyToManyField(
        Student,
        related_name="assignments_as_student",
        help_text="Students assigned to this work.",
    )
    ...
    set_date = DateField(
        default=timezone.now, help_text="The date when the homework was assigned."
    )
    due_date = DateField(help_text="The deadline for the homework.")
...
class AssignmentAttempt(Model):
    ...
    submitted_files = ManyToManyField(
        TeachingResource, blank=True, related_name="assignment_attempts"
    )
    ...
    class Meta:
        unique_together = (
            "assignment",
            "student",
        )  # Prevent multiple attempts if needed
...
class Feedback(Model):
    ...
    assignmentAttempt = ForeignKey(
        AssignmentAttempt, on_delete=CASCADE, related_name="feedback_entries", null=True
    )
    ...
    submitted_files = ManyToManyField(
        TeachingResource, blank=True, related_name="feedbacks"
    )
