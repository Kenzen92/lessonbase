...
from apps.assignments.serialisers import (
    ...
    AssignmentAttemptDetailsSerializer,
...
class AssignmentAttemptViewSet(ModelViewSet):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"message": "Submission successful"}, status=status.HTTP_201_CREATED
            )
        else:
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path=r"(?P<assignment_id>\d+)/students/(?P<student_id>\d+)/attempt",
    )
    def retrieve_by_assignment_and_student(
        self, request, assignment_id=None, student_id=None
    ):
        attempt = get_object_or_404(
            AssignmentAttempt, assignment_id=assignment_id, student_id=student_id
        )
...
class FeedbackViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        graded = request.data.get(
            "graded", True
        )  # Default to True when providing feedback
    ...
    def list(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Assigment attempt does not exist"},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        if isinstance(user, Teacher):
            feedbacks = Feedback.objects.filter(
                teacher=user, assignmentAttempt=assignmentAttempt
            )
        else:
            feedbacks = Feedback.objects.filter(
                student=user, assignmentAttempt=assignmentAttempt
            )
