...
class ChatListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        # Debug prints
        print(
            f"Chat created with participants: {[participant.id for participant in chat.participants.all()]}"
        )
        print(
            f"Chat participants (excluding current user): {[participant.id for participant in chat.participants.exclude(id=self.request.user.id)]}"
        )
